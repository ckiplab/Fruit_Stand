#----#
#usage: Deduction mechanism on falsehoods, balance version (compensation on certain claims)
#----#
import logging
import time
import os
...
from detect_liar import IsLiar, IsSuperlative
...
def generate_dialog(starter, follower, dataset, args, record_aspect='Seller', max_round=10)->list:
    ...
    quality_list = dataset['quality_profit'][:3]
    ...
    while round_ < max_round and not decided:
        ...
        round_ += 1
    return output_list_seller, output_list_buyer, verbose_list, quality_list #3 lists of dictionary, 1 list of int
...
def generate_sampled_dialogs(dataset, starter:Agent, follower:Agent, 
    ...
    while len(dialogs_list_seller) < args.batch_size:
        pair_list_seller, pair_list_buyer, c_list, quality_list = generate_dialog(starter, follower, dataset, args,  max_round=10)
        
        #liar detection
        seller_reply = [x['labels'] for x in pair_list_seller]
        if IsLiar(seller_reply, quality_list):
            for dictionary in pair_list_seller:
                dictionary['reward'] = dictionary['reward'] -2.0
            
        #balance 
        for dictionary in pair_list_seller:
            if IsSuperlative(dictionary['labels']):
                dictionary['reward'] = dictionary['reward'] +0.5
...
def main():
    ...
    file_prefix_seller = make_logdir(f'Liar_Balance_Both_{args.reward_baseline_type}_b{args.batch_size}_lr{args.lr}/Seller/epoch_00')
    file_prefix_buyer = make_logdir(f'Liar_Balance_Both_{args.reward_baseline_type}_b{args.batch_size}_lr{args.lr}/Buyer/epoch_00')
