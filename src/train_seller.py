#---#
#usage: fine-tune pretrained (T5) model from a seller's perspective.
#---#
import os
import math
import logging
...
SPECIAL_TOKENS = ["<bos>", "<eos>", "<Buyer>", "<Seller>","<sep>", "<pad>"]
ATTR_TO_SPECIAL_TOKEN = {'bos_token': '<bos>', 'eos_token': '<eos>', 'pad_token': '<pad>', 'sep_token': '<sep>',
                         'additional_special_tokens': ['<Buyer>', '<Seller>']}
...
def build_input_from_segments( scenario, history, reply, tokenizer, with_eos=True):
    """ Build a sequence of input from 3 segments: scenario, history and last reply. """
    bos, eos, speaker1, speaker2,_ = tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS[:-1])
...
def train():
    ...
    def update(engine, batch):
        ...
        #token_tpye_ids is for BERT or GPT2
    ...
    if args.local_rank in [-1, 0]:
        ...
        checkpoint_handler = Checkpoint(objects_to_checkpoint, DiskSaver(f'{log_dir}'), n_saved=args.n_epochs,global_step_transform=gst, score_name=f'_avgPPL_{metrics["average_ppl"]}')
