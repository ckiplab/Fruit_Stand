...
class Agent:
    def __init__(self, args, aspect='Buyer'):
        ...
        #for PG and SL
        self.init_size = self.get_num_of_generation(args)
        self.first_talks = self.get_beginner_from_random(self.init_size)
    ...
    def get_num_of_generation(self,args)->int: 
        if self.aspect=='Seller' and args.number_of_scenario%2!=0:
            return int(args.number_of_scenario/2)+1
        return int(args.number_of_scenario/2)
    
    def get_beginner_from_random(self, num_of_generation=5):
        ...
        def generate_scenario_buyer(preference_list): 
            scenario_list = [f'Preference of {FRUITS[i]} is {preference_list[i]} <sep>' for i in range(3)]
            '''
            ['Preference of apples is 1 <sep>',
            'Preference of bananas is 2 <sep>',
            'Preference of oranges is 3 <sep>']
            '''
            return scenario_list

        def generate_scenario_seller(quality_list, profit_list):
            scenario_list = [f'Quality of {FRUITS[i]} is {quality_list[i]} <sep>' for i in range(3)]
            scenario_list = scenario_list+[f'Profit of {FRUITS[i]} is {profit_list[i]} <sep>' for i in range(3)]
            return scenario_list

        def generate_input_ids_str(scenario_list)->str:
            scenario = ' '.join(scenario_list)

            return '<bos> '+scenario+ ' <eos>'

        def get_reward_buyer(preference_list, quality_list)->list:
            return [int(preference_list[i])*int(quality_list[i]) for i in range(3)]

        def get_reward_seller(preference_list, quality_list, profit_list)->list:
            return [int(preference_list[i])*int(quality_list[i])+int(profit_list[i]) for i in range(3)]

        def get_max_reward(reward_buyer:list, reward_seller:list):
            ...
            max_reward_b = [FRUITS[x] for x in get_indices(max_value_b, reward_buyer)]
            max_reward_s = [FRUITS[x] for x in get_indices(max_value_s, reward_seller)]
        ...
        def generate_initial_condition()->dict:
            preference_list = [random.randint(1,MAX_PREFERENCE) for _ in range(3)]
            quality_list = [random.randint(1,MAX_QUALITY) for _ in range(3)]
            profit_list = [random.randint(1,MAX_PROFIT) for _ in range(3)]

            scenario_buyer = generate_scenario_buyer(preference_list)
            scenario_seller= generate_scenario_seller(quality_list, profit_list)
            reward_buyer = get_reward_buyer(preference_list, quality_list)
            reward_seller= get_reward_seller(preference_list, quality_list, profit_list)
            qp_list  = quality_list+profit_list
            starts = self.aspect
            max_reward_b, max_reward_s, win_win_choice = get_max_reward(reward_buyer, reward_seller)
            ...
            if self.aspect == 'Buyer':
                input_ids = tokenize(generate_input_ids_str(scenario_buyer))
                scenario  = tokenize(scenario_buyer)
                scenario_other = tokenize(scenario_seller)
            else:
                input_ids = tokenize(generate_input_ids_str(scenario_seller))
                scenario  = tokenize(scenario_seller)
                scenario_other = tokenize(scenario_buyer)
            ...
            out_dict = {'input_ids':input_ids,
                        ...
                        'quality_profit':qp_list,
                        'preference':preference_list,
                        'starts':starts,
                        'max_reward_b':max_reward_b,
                        'max_reward_s':max_reward_s,
                        'win_win_choice':win_win_choice}

            return out_dict
             
        #Prepare the random generated  dataset
        print(f"Build inputs started by {self.aspect}")
        
        
        #build dataset of initial conditions
        datasets = [generate_initial_condition() for _ in range(num_of_generation)]
...
def generate_dialog(starter, follower, dataset, args, record_aspect='Seller', max_round=10)->list:
    _input_ids = dataset['input_ids']
    scenario_starter = dataset['scenario']
    scenario_follower = dataset['scenario_other']
    max_score_s_selection = dataset['max_reward_s']#list of str
    max_score_b_selection = dataset['max_reward_b']#str: apples/bananas/oranges
...
def generate_batch_sampled_dialogs(input_dict, Buyer:Agent, Seller:Agent, args):
    ...
    return batch_sampled_dialogs_seller, batch_sampled_dialogs_buyer, verbose_list, time_passed
...
def write_dialogs(batch_dialogs, baseline_reward, file_prefix, args, epoch):
    ...
    file_path = f'{file_prefix}/dialogs_for_RL_Random_{epoch}.txt'
...
def print_dialogs(info, batch_dialogs, file_prefix, args, epoch, dialog_num):
    ...
    Path(f"./{file_prefix}").mkdir(parents=True, exist_ok=True)
    file_path = f'{file_prefix}/dialogs_for_RL_Random_{epoch}_with_n.txt'
    if os.path.isfile(file_path):
        with open(file_path, 'a', encoding='utf-8') as f:
            f.write('---------------------------- batch separation (input condition changed)-------------------\n')
...
def main():
    ...
    parser.add_argument("--lr", type=float, default=6.25e-7, help="Learning rate")
    parser.add_argument("--value_only", action='store_true', help="If assigned get rid of description quality... profit...")
    parser.add_argument("--batch_size", type=int, default=32, help="Batch size for training")
    parser.add_argument("--max_generate_length", type=int, default=32, help="Number of max_generate_length")
    ...
    parser.add_argument("--number_of_scenario", type=int, default=423, help="number of randomly generated scenario")
    ...
    file_prefix_seller = make_logdir(f'RL_Both_Random_{args.reward_baseline_type}_b{args.batch_size}_lr{args.lr}/Seller/epoch_00')
    file_prefix_buyer = make_logdir(f'RL_Both_Random_{args.reward_baseline_type}_b{args.batch_size}_lr{args.lr}/Buyer/epoch_00')
    ...
    for epoch in range(args.n_epochs):
        ...
        for i,input_dict in enumerate(all_first_talks):
            flatted_total_sampled_dialogs_seller, flatted_total_sampled_dialogs_buyer, verbose_list, time_passed = generate_batch_sampled_dialogs(input_dict, Buyer, Seller, args)
            if args.write_dialogs:
