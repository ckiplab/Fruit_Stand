#----#
#usage: Deduction mechanism on falsehoods, original version 
#----#
import logging
import time
import os
...
from detect_liar import IsLiar
    ...
    while len(dialogs_list_seller) < args.batch_size:
        ...
        if IsLiar(seller_reply, quality_list):
            for dictionary in pair_list_seller:
                dictionary['reward'] = dictionary['reward'] -args.liar_minus
...
def main():
    ...
    parser.add_argument('--liar_minus', type=float, default=1.0, help='The minus score on liar detection')
    ...
    file_prefix_seller = make_logdir(f'Liar_deduction_RL_Both_-{args.liar_minus}_b{args.batch_size}_lr{args.lr}/Seller/epoch_00')
    file_prefix_buyer = make_logdir(f'Liar_deduction_RL_Both_-{args.liar_minus}_b{args.batch_size}_lr{args.lr}/Buyer/epoch_00')
